## Attribution 
This course summary is borrowed from <a href="https://github.com/mbadry1" target="_blank">Mahmoud Badry</a> summaries of the Deep Learning specialization at [Coursera](https://www.coursera.org/specializations/deep-learning) which is moderated by [DeepLearning.ai](http://deeplearning.ai/). 
...
Some of the applications of computer vision that are using deep learning include:
  - Self driving cars.
  - Face recognition.
  - New Art Generation
 
Deep leaning techniques are always evolving making a new architectures which can help us in other areas other than computer vision. For example, Andrew Ng took some ideas of computer vision and applied it in speech recognition.
...
### Padding
 ...
 To give it a general rule, if a matrix `nxn` is convolved with `fxf` filter/kernel give us `n
 ...
 The general rule now,  if a matrix `nxn` is convolved with `fxf` filter/kernel and padding `p` give us `n+2p
 If n ' 6, f ' 3, and p ' 1 Then the output image will have `n+2p
 ...
 In computer vision f is usually odd. Some of the reasons is that it has a center value.
